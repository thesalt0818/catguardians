    ...
    execute: function () {
      ...
      var BuildingManager = exports('BuildingManager', (_dec = ccclass('BuildingManager'), _dec2 = property(SpriteFrame), _dec3 = property(SpriteFrame), _dec4 = property(ProgressBar), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property([SpriteFrame]), _dec8 = property(Label), _dec9 = property(PopupControl), _dec(_class = (_class2 = /*#__PURE__*/function (_Component) {
        ...
        function BuildingManager() {
          ...
          _initializerDefineProperty(_this, "buildTime", _descriptor5, _assertThisInitialized(_this));
          ...
          // 업그레이드 팝업
          _initializerDefineProperty(_this, "levelUpInterval", _descriptor9, _assertThisInitialized(_this));
          // 몇 초마다 1레벨씩 오를지
          _this.guideText = null;
          _this.sprite = null;
          _this.isBuilding = false;
        ...
        _proto.playFloatingAnimation = function playFloatingAnimation() {
          ...
          var upPos = new Vec3(startPos.x, startPos.y + 10, startPos.z); // 10만큼 위로
        ...
        // [변경] 마우스 오버: 공사 전(A 이미지)일 때만 작동
        ;

        _proto.onMouseEnter = function onMouseEnter() {
          if (this.isCompleted) return; // 완성 후에는 오버 효과 무시

          // 마우스 올리면 불투명하게 + 크기 키우기
        ...
        _proto.onMouseLeave = function onMouseLeave() {
          ...
          // 마우스 나가면 다시 반투명하게 (단, 공사 시작 전일 때만)
          this.setSpriteAlpha(this.SEMI_TRANSPARENT);
          tween(this.node).to(0.1, {
        ...
        _proto.onBuildingClick = function onBuildingClick() {
          // [추가] 클릭하는 순간 가이드 텍스트 숨기기
          if (this.guideText && this.guideText.active) {
            this.guideText.active = false;
          }

          // 1. 이미 완성되었다면? 클릭 애니메이션 + 아이콘 보여주기
          if (this.isCompleted) {
            this.playClickAnimation(); // 클릭 효과 실행
            this.showSubButtons();
            //this.setSpriteAlpha(this.OPAQUE);
            return;
          }
          ...
          if (this.progressBar) this.progressBar.node.active = true;
        }

        // [추가] C 이미지 클릭 시 꿀렁이는 효과
        ;
        ...
        _proto.update = function update(dt) {
          //this.setSpriteAlpha(this.OPAQUE);
          if (this.isBuilding && this.timer < this.buildTime) {
            this.timer += dt;
            ...
            // 10레벨 단위(10, 20...)가 되면 레벨업 중지하고 업그레이드 대기
        ...
        _proto.finishBuilding = function finishBuilding() {
          ...
          this.setSpriteAlpha(this.OPAQUE);
          //if (this.imageC) this.sprite.spriteFrame = this.imageC;

          // 레벨에 맞는 이미지로 변경
          this.applyUpgradedImage();
          if (this.progressBar) this.progressBar.node.active = false;

          // 완성되는 순간 스케일을 원래대로 초기화 (혹시 마우스 오버 상태였다면)
      ...
      }), _descriptor5 = _applyDecoratedDescriptor(_class2.prototype, "buildTime", [property], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor6 = _applyDecoratedDescriptor(_class2.prototype, "upgradeImages", [_dec6, _dec7], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor9 = _applyDecoratedDescriptor(_class2.prototype, "levelUpInterval", [property], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
